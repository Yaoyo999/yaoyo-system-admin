      ...
      <el-form ref="dishRef" :model="form" :rules="rules" label-width="80px">
        ...
        >
          ...
          <el-table-column
            ...
            width="50"
          />
          <el-table-column label="口味名称" prop="name" width="250">
            <template #default="scope">
              <el-select
...
function changeFlavorName(row) {
  ...
  checkValueList.value = dishFlavorListSelect.value.find(
...
function submitForm() {
  proxy.$refs["dishRef"].validate((valid) => {
    if (valid) {
      if (form.value.dishFlavorList.length != 0) {
